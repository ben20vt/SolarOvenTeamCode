...
%% 
TimeEnd = max(PosixTime_Calculated);  %Calculates the Unix timestamp of the end date 
TimeStart = min(PosixTime_Calculated);    %Calculates the Unix timestamp of the start date 
...
fprintf('The maximum temperature difference during testing was: %d °C and occured on %s.  \n',Max,dateatmax);    %Displays the max delta T and time of occurance
fprintf("This means on %s the oven reached its greatest temperature above the ambient temperature during testing, and took %d seconds to reach this temperature. \n",dateatmax,timetomax);
%% Calculating Efficiency
    %% Qab
    Tf = Max;    %Calculates the average temp difference from the data
    DeltaT = Tf;  %Converts the temperature difference to kelvin
    ...
    t = (timeatmax - TimeStart);  %duration to max diff in temp in hours
...
tdiffjuly = (n*ijuly*AreaM*t)/(Rho*VolM*heatCap*12);  %Theoretical max temp difference in July in Blacksburg
...
vars={'Dates','Unix Timestamp (s)','Oven Temperature (°C)','Ambient Temperature (°C)','Temperature Difference (°C)'};
